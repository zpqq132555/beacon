...
- `beacon init` 检测 Codex 插件缓存中已安装的 Superpowers（`~/.codex/plugins/cache/...`），避免重复安装（[#115](https://github.com/zpqq132555/beacon/pull/115)）。
...
公开版曾扩展 Kimi Code 相关安装、OpenSpec tool 集成、Superpowers agent 映射与检测能力（[#90](https://github.com/zpqq132555/beacon/pull/90)）。私有版当前可用平台范围以 README 中的首批私有版平台和团队供应链配置为准。

### `beacon uninstall` 命令

新增 `beacon uninstall [path]` 安全移除 Beacon 分发的 skills、rules、hooks，支持 `--scope`、`--force`、`--json`。卸载逻辑按当前安装目标和 Beacon 管理标记清理产物，避免误删用户自定义内容（[#95](https://github.com/zpqq132555/beacon/issues/95)）。
...
- `beacon update` 的包来源由 Beacon 供应链配置决定，未配置私有版本源时不会静默回退到公开 registry（[#100](https://github.com/zpqq132555/beacon/issues/100)）。
- 启动时显示版本，并在配置了私有版本元数据来源时检查是否有新版本（[#99](https://github.com/zpqq132555/beacon/issues/99)）。
...
- OpenSpec 制品按 `openspec instructions ... --json` 加载 context/rules/template（[#66](https://github.com/zpqq132555/beacon/issues/66)）。
- Pi slash 命令注册与生命周期（[#89](https://github.com/zpqq132555/beacon/issues/89)）。
- 符号链接安全的卸载与文件复制（[#85](https://github.com/zpqq132555/beacon/issues/85)）。
