...
新增 Kimi Code 为第 29 个支持平台，覆盖 `.kimi-code/` 下的项目/全局 skill 安装、OpenSpec `kimi` 工具集成、Superpowers `kimi-code-cli` 映射与检测（[#90](https://github.com/rpamis/beacon/pull/90)）。

### `beacon uninstall` 命令

新增 `beacon uninstall [path]` 安全移除 Beacon 分发的 skills、rules、hooks，支持 `--scope`、`--force`、`--json`。覆盖 29 个平台、7 种 hook 格式、3 种 rule 格式，仅清理 Beacon 管理的产物（[#95](https://github.com/rpamis/beacon/issues/95)）。
...
- `beacon update --registry https://registry.npmjs.org` 强制走官方源（[#100](https://github.com/rpamis/beacon/issues/100)）。
- 启动时显示版本并检查 npm registry 是否有新版本（[#99](https://github.com/rpamis/beacon/issues/99)）。
...
`beacon init` 和 `beacon update` 现在支持一键安装 [CodeGraph](https://github.com/colbymchenry/codegraph)（`@colbymchenry/codegraph`），为 Agent 提供语义代码索引能力。自动检测 7 个支持平台（Claude Code、Cursor、Codex、OpenCode、Gemini、Kiro、Antigravity），安装 CLI 并初始化项目索引。`beacon doctor` 可检查 CodeGraph 状态。
