...
| `/beacon-init` | 辅助维护 | Beacon | 项目级 AGENTS 树维护（手动全量维护 / archive 确认后转调） |
...
- `/beacon-init` 是项目级 AGENTS 树维护入口，不属于五阶段主流程。
- 手动调用 `/beacon-init` 时，按当前工作区执行全量维护。
- archive 只在 `beacon/reference/agents-sedimentation.md` 判定值得沉淀时，先给摘要建议，再在用户确认后转调 `/beacon-init`。
- AGENTS 节点如何分层，按 `beacon/reference/agents-topology.md` 执行。
