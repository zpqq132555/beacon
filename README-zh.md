...
- **如何做跨平台 Skill 分发和安装** — Beacon 支持多种 AI 编码平台、项目级/全局安装、中文/英文 Skill 选择，以及平台差异化目录（例如
...
> 后续升级 Beacon 本身：执行 `beacon update` 或 `npm install -g beacon@latest`。

## 对OpenClaw和Hermes、或其他AI平台的支持

对于直接使用通用 `skills` CLI 的平台，可以用下面的方式安装 Beacon skill 包：
